    ...
    for positions in antenna_map.antenna_positions.values():
        for i in range(len(positions)):
            ...
            for j in range(i + 1, len(positions)):
                ...
                while 0 <= arow1 < height and 0 <= acol1 < width:
                    antinode_map[arow1][acol1] = True
                    arow1, acol1 = arow1 - row_delta, acol1 - col_delta

                arow2, acol2 = row2 + row_delta, col2 + col_delta
                while 0 <= arow2 < height and 0 <= acol2 < width:
                    antinode_map[arow2][acol2] = True
                    arow2, acol2 = arow2 + row_delta, acol2 + col_delta

    for positions in antenna_map.antenna_positions.values():
        if len(positions) < 1:
            continue
        for i, j in positions:
            antinode_map[i][j] = True
