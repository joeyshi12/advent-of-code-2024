...
def fix_update_order(update: list[int],
                     less_than_rule_map: dict[int, set[int]],
                     greater_than_rule_map: dict[int, set[int]]):
    while not is_valid_update(update, less_than_rule_map, greater_than_rule_map):
        for i in range(len(update) - 1):
            if update[i] not in greater_than_rule_map:
                continue
            greater_than_rules = greater_than_rule_map[update[i]]
            for j in range(i + 1, len(update)):
                if update[j] in greater_than_rules:
...
def main():
    ...
    for update in updates:
        if not is_valid_update(update, less_than_rule_map, greater_than_rule_map):
            fix_update_order(update, less_than_rule_map, greater_than_rule_map)
            middle_page_number_sum += update[len(update) // 2]

    print(f"Invalid middle page sum: {middle_page_number_sum}")
